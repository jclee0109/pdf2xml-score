...
```python
def crop_piano_both(page_img, system: SystemInfo, treble_id: str, bass_id: str) -> Image:
    # active_parts 내 인덱스 기준
    treble_idx = system.active_parts.index(treble_id)
    bass_idx   = system.active_parts.index(bass_id)
    system_h = system.y_bottom_px - system.y_top_px
    n = len(system.active_parts)
    part_h = system_h / n
    y_top = system.y_top_px + part_h * treble_idx - 20  # 코드 심볼 여백
    y_bot = system.y_top_px + part_h * (bass_idx + 1) + 5
...
```python
def crop_piano(page_img, system: SystemInfo, part_id: str) -> Image:
    # active_parts 내 인덱스 기준 (global PartInfo.order가 아님)
    # 쉬는 파트가 생략되면 active_parts 순서가 달라지므로 반드시 이 인덱스를 써야 함
    idx = system.active_parts.index(part_id)
    system_h = system.y_bottom_px - system.y_top_px
    n_parts = len(system.active_parts)
    part_h = system_h / n_parts

    piano_y_top = system.y_top_px + part_h * idx - 20   # 코드 심볼 여백 포함
    piano_y_bot = system.y_top_px + part_h * (idx + 1) + 5
...
class MusicXMLBuilder:
    ...
    def _build_part(self, part, score):
        ...
        # 코드 심볼은 "Piano treble" 파트에만 삽입
        # part.name == "Piano"는 treble/bass 분리 이후로 항상 false → 버그
        chords = score.chords if part.name == "Piano treble" else []
        # 마디별로 <measure> 생성
        # needs_review=True인 항목은 XML 주석으로 마킹 (<!-- REVIEW: ... -->)

    def _validate_schema(self, root):
        # lxml로 MusicXML 4.0 XSD 검증 (ET.tostring 아님)
        xsd = etree.XMLSchema(etree.parse("musicxml.xsd"))
        if not xsd.validate(root):
            for err in xsd.error_log:
                log.error(f"MusicXML schema error: line {err.line} — {err.message}")
            raise ValueError("MusicXML schema validation failed")
```
